...
namespace BigIntInterface
{
    ...
    public struct BigInt : IComparable<BigInt>, IEquatable<BigInt>
    {
        ...
        public BigInt(BigInt rhs) : this(rhs.bytes == null? new List<byte>() : new List<byte>(rhs.bytes), rhs.sign) {}
        ...
        public static explicit operator BigInt(bool val) { return new BigInt(val? One : Zero); }
        public static explicit operator BigInt(string str) { return new BigInt(str); }
        public static implicit operator BigInt(long val) { return new BigInt(val); }

        public static explicit operator long(BigInt a) { return a.ToLong(); }
        public static explicit operator int(BigInt a) { return a.ToInt(); }
        ...
        public override string ToString() { return ToDec(); }
        ...
        public static BigInt operator &(BigInt lhs, BigInt rhs)
        {
            if (lhs.IsZero || rhs.IsZero)
                return new BigInt();
            int min = Math.Min(rhs.ByteSize, lhs.ByteSize);
        ...
        void LshEqualsImpl(int rhs, int size)
        {
            ...
            if (rhs < 0)
                throw new BigIntException("Negative shift");
        ...
        public void RshEquals(int rhs)
        {
            ...
            if (bigShift >= ByteSize)
            {
                SetFromZero();
                return;
            }
            int newSize = ByteSize - bigShift;
        ...
        public static bool operator !=(BigInt rhs, BigInt lhs) { return !(rhs == lhs); }
        ...
        public bool Equals(BigInt bi) { return bi == this; }
        ...
        public static int AbsCmp(BigInt lhs, BigInt rhs)
        {
            ...
            for (int i = lhs.ByteSize - 1; i >= 0; --i)
            {
                if (lhs.bytes[i] < rhs.bytes[i])
                    return -1;
                if (lhs.bytes[i] > rhs.bytes[i])
        ...
        {
            AddImpl(op, a, b, ref res, a.ByteSize, b.ByteSize);
        }
        static BigInt AddImpl(Op op, BigInt a, BigInt b, ref BigInt res, int aSize, int bSize)
        {
        ...
        public static BigInt operator +(BigInt lhs, BigInt rhs) { return Add(Op.Add, lhs, rhs); }
        public static BigInt operator -(BigInt lhs, BigInt rhs) { return Add(Op.Sub, lhs, rhs); }
        ...
        public static void DivRem(BigInt a, BigInt b, ref BigInt q, out BigInt r)
        {
            ...
            if (q.bytes == b.bytes)
                throw new BigIntException("divRem: b, q and r must be " + "references to different objects");
            if (AbsCmp(b, a) > 0)
            {
            ...
            var bSize = b.ByteSize;
            var aSize = a.ByteSize;

            // we have r < b, so we allocate memory for the worst case
            var rSize = bSize + 1;
            var qSize = aSize;
        ...
        public void SplitNum(out BigInt low, out BigInt high, int n)
        {
            ...
            high = new BigInt(ByteSize - n, Sign.Positive);
            // low.Resize(n);
            // high.Resize(ByteSize - n);
        ...
        static void MulImpl(BigInt a, BigInt b, ref BigInt res)
        {
            if (a.ByteSize <= 1)
            {
                if (a.IsZero)
                    ...
                    SmallMulImpl(b, a.bytes[0], ref res);
                return;
            }
            if (b.ByteSize <= 1)
            {
                if (b.IsZero)
                    ...
                    SmallMulImpl(a, b.bytes[0], ref res);
                return;
            }
            var m = Math.Min(a.ByteSize, b.ByteSize);
            var m2 = m / 2;
        ...
        public static BigInt Sgn(BigInt a) => a.sign;
        // http://lib.tkk.fi/Diss/2005/isbn9512275279/article3.pdf
        public static BigInt Sqrt(BigInt n)
        {
            if (n.sign == Sign.Negative)
                throw new BigIntException("Can't calculate the sqrt of a negative number");
            if (n.IsZero)
                return n;

            BigInt root = Zero;
            BigInt mask = new BigInt(n.ByteSize, Sign.Positive);
            mask.bytes[n.ByteSize - 1] = 1 << (8 - 2); // biggest power of 4 possible
            BigInt remainder = new BigInt(n);
            BigInt rootPlusMask;

            while (!mask.IsZero)
            {
                rootPlusMask = root + mask;
        ...
        public static BigInt Rand(BigInt n)
        {
            if (n.sign != Sign.Positive) throw new BigIntException("Argument out of range");
            List<byte> bytes = new List<byte>(Enumerable.Repeat((byte) 0, n.ByteSize));
            BigInt r;

            do 
            {
                for (int i = 0; i < bytes.Count; ++i) bytes[i] = (byte) random.Value.Next(256);
                r = new BigInt(bytes);
                r.ClearZeroes();
            } while (AbsCmp(r, n) >= 0);
