...
WindowHandler::WindowHandler(const char* title, Scene& scene,
    ...
    if (theInstance != nullptr) {
        error(std::logic_error("Multiple instances of WindowHandler "
                               "aren't allowed"));
    }
    theInstance = this;
    scene.onStart();
...
HICON createIcon(const ConstPaletteSprite<Width, Height, CP>& sprite,
    ...
    struct RaiiBitmap {
        ...
        RaiiBitmap(int width, int height,
            ...
            if (!val)
                error(WinapiError("CreateBitmap"));
        }
    ...
    if (res == nullptr)
        error(WinapiError("CreateIconIndirect"));
...
HWND WindowHandler::createWindow(const char* title, Rect r,
    ...
    if (hInstance == nullptr)
        error(WinapiError("GetModuleHandle()"));
    WNDCLASSW wc;
    ...
    if (!::RegisterClassW(&wc)) {
        error(WinapiError("RegisterClassW"));
    }
    ...
    if (hwnd == nullptr) {
        error(WinapiError("CreateWindow"));
    }
    WindowDC dc(hwnd);
    ...
    if (pf == 0) {
        error(WinapiError("ChoosePixelFormat"));
    }

    if (!::SetPixelFormat(dc, pf, &pfd)) {
        error(WinapiError("SetPixelFormat()"));
    }
    return hwnd;
}
