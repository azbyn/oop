...
T selectObject(HDC hdc, T t) {
    ...
    if (t == nullptr || t == HGDI_ERROR)
        error(WinapiError("SelectObject"));
    return res;
}
template<class T>
void deleteObject(T t) {
    if (!::DeleteObject(t))
        error(WinapiError("DeleteObject"));
}
...
Paint::Paint(HDC ogHDC, Point size)
    ...
    bitmap = ::CreateDIBSection(
        ...
        (void**)&data, nullptr, 0);
    if (bitmap == nullptr)
        error(WinapiError("CreateDIBSection"));
    h.biHeight = height;
...
void stretchBlt(HDC dst, Paint::StretchData::SzRect dstRect,
    ...
    if (!::StretchBlt(dst, dstRect.x, dstRect.y, dstRect.w, dstRect.h,
                      ...
                      SRCCOPY)) {
        error(WinapiError("StretchedBlt"));
    }
}
